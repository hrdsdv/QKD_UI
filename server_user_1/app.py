        ...
        try:
            ...
            if qkd_socket_client:
                try:
                    ...
                    log_to_file(f"[WebSocket Client] Отправлены данные просеивания на server_2 для {sequence_id}, mismatches={mismatches}, qber={qber_value}%", level="INFO")
                    print(f"[WebSocket Client] Отправлены данные просеивания на server_2 для {sequence_id}")
                    ...
                    try:
                        log_to_file(f"[WebSocket Client] Пытаемся переподключиться к server_2...", level="INFO")
                        init_qkd_socket_client()
                        if qkd_socket_client:
                            qkd_socket_client.emit('sifting_complete', sifting_data, wait=True)
                            log_to_file(f"[WebSocket Client] Переподключен и отправлены данные просеивания на server_2 для {sequence_id}", level="INFO")
                            print(f"[WebSocket Client] Переподключен и отправлены данные")
                    except Exception as reconnect_error:
                        log_to_file(f"[WebSocket Client] Не удалось переподключиться к server_2: {reconnect_error}", level="ERROR")
                ...
                try:
                    ...
                    if qkd_socket_client:
                        qkd_socket_client.emit('sifting_complete', sifting_data, wait=True)
                        log_to_file(f"[WebSocket Client] Инициализирован и отправлены данные просеивания на server_2 для {sequence_id}", level="INFO")
