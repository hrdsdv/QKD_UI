...
def select_sequence():
    ...
    if qkd_module.select_sequence(sequence_id, username):
        ...
        # Отправляем данные клиентам server_2 через WebSocket
        try:
            socketio.emit('sifting_complete', sifting_data, namespace='/')
            log_to_file(f"Отправлены данные просеивания клиентам server_2 для {sequence_id}, mismatches={mismatches}, qber={qber_value}%", level="INFO")
        except Exception as e:
            log_to_file(f"Ошибка отправки данных просеивания клиентам server_2: {e}", level="ERROR")
        
        # Отправляем данные на server_1 через WebSocket клиент
        try:
            ...
            try:
                ...
                log_to_db(db_path, None, 'QKDModule', 'ERROR', f"Ошибка WebSocket синхронизации с server_1: {e}")
            except:
                pass
...
def init_qkd_socket_client():
    ...
    try:
        ...
        def on_connect():
            log_to_file("[WebSocket Client] Подключен к серверу 1 через WebSocket", level="INFO")
            print("[WebSocket Client] Подключен к серверу 1")
        
        @qkd_socket_client.on('disconnect')
        def on_disconnect():
            log_to_file("[WebSocket Client] Отключен от сервера 1 через WebSocket", level="WARNING")
            print("[WebSocket Client] Отключен от сервера 1")
